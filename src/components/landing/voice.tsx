...
/**
 * Three voices from the agency floor. Names, agencies and initials come from
 * the data module; the quotes and roles are translated (aligned by index).
 */
export function Voice() {
  const tr = useTranslations("voice");
  const quotes = tr.raw("items") as { quote: string; role: string }[];
    ...
    <section className="relative overflow-hidden border-t border-white/[0.06] py-24 md:py-32">
      ...
      <div className="mx-auto max-w-content px-5 md:px-8">
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-[1.35fr_1fr]">
          ...
          <Reveal delay={140}>
            <FeatureCard person={t1} quote={quotes[0].quote} role={quotes[0].role} />
          </Reveal>
          ...
          <div className="flex flex-col gap-6">
            {t2 && (
              <Reveal delay={200}>
                <SmallCard person={t2} quote={quotes[1].quote} role={quotes[1].role} />
              </Reveal>
            )}
            {t3 && (
              <Reveal delay={260}>
                <SmallCard person={t3} quote={quotes[2].quote} role={quotes[2].role} />
              </Reveal>
            )}
          </div>
        </div>
  ...
  return (
    <figure className="relative flex h-full flex-col rounded-3xl border border-white/[0.08] bg-white/[0.03] p-8 md:p-10">
      ...
      <figcaption className="mt-8 flex items-center gap-3">
        ...
        <div>
          <p className="text-[13.5px] font-semibold text-paper">{person.name}</p>
  ...
  return (
    <figure className="flex h-full flex-col justify-between rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 md:p-7">
      ...
      <figcaption className="mt-6 flex items-center gap-3">
        ...
        <div>
          <p className="text-[12.5px] font-semibold text-paper">{person.name}</p>
